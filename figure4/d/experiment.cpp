...
int main(int argc, char** argv) {
    ...
    for (int k = 0; k < SEEDS_N; k++) {
        if ((pid = fork()) == 0) {
            ...
            float omega_init = 0;
            do {
                b_init = b_gen(gen) / 1000.;
                omega_init = (omega_gen(gen) * 2. * 3.14159) / 1000.;
            ...
            for (float lr : learning_rates) {
                for (float beta : surrogate_betas) {
                    for(std::vector<bool> comb : combinations) {
                        std::string file_prefix = "./results/mode_" + std::to_string(lr) + "_"  + std::to_string(beta) + "_" + std::to_string(comb[0]) + std::to_string(comb[1]) + std::to_string(comb[2]) + std::to_string(comb[3]) + std::to_string(comb[4]);
                        ...
                        student = new Lrf(excitatory_n, inhibitory_n, v_reset_init_, i_reset_init_, 1., b_init_, omega_init_, use_adam, lr);
                        if (comb[0]) {
                            student->ws = ws_copy;
                        } else {
                        ...
                        for (long i = 0; i < steps; i++) {
                            ...
                            if (teacher_out != student_out) {
                                student->training_step(student_out - teacher_out, comb[0], comb[1], comb[2], comb[3], comb[4], use_lambda, beta);
                            }
