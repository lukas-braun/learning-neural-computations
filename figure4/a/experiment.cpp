    ...
    for (bool use_lambda : {false, false, true}) {    
        for (int k = 0; k < SEEDS_N; k++) {
            if ((pid = fork()) == 0) {
                ...
                Lif *student = new Lif(excitatory_n, inhibitory_n, 0., v_reset_init, 1., tau_s_init, tau_m_init, use_adam, 1e-4);
                ...
                for(std::vector<bool> comb : combinations) {
                    std::string file_prefix = "./results/mode_" + std::to_string(run) + "_" + std::to_string(comb[0]) + std::to_string(comb[1]) + std::to_string(comb[2]) + std::to_string(comb[3]);
                    ...
                    student = new Lif(excitatory_n, inhibitory_n, 0., v_reset_init_, 1., tau_s_init_, tau_m_init_, use_adam, 1e-4);
                    if (comb[0]) {
                        student->ws = ws_copy;
                    } else {
                    ...
                    for (long i = 0; i < steps; i++) {
                        ...
                        if (teacher_out != student_out) {
                            student->training_step(student_out - teacher_out, comb[0], comb[1], comb[2], comb[3], use_lambda, surrogate_betas[run]);
                        }
