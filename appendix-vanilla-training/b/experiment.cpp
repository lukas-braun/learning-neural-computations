...
int main(int argc, char** argv) {
    ...
    for (int k = 0; k < SEEDS_N; k++) {
        if ((pid = fork()) == 0) {
            ...
            } while (teacher->beta > 2.5 and target_firing_rate < 10.);
            inputs = new Poisson(excitatory_n, inhibitory_n, 10., 40., ++seed);
            teacher->reset();
            
            // Init Student
            Lrf *student = new Lrf(excitatory_n, inhibitory_n, v_reset_gen(gen), i_reset_gen(gen), 1., 0., 0., use_adam, 1e-4);
            do {
            ...
            for (long i = 0; i < steps; i++) {
                ...
                if (teacher_out != student_out) {
                    student->training_step(student_out - teacher_out, true, true, true, true, true, use_lamda, 0.);
                }
